...
async function tailor({ resume, company, jobDescription, portfolio = "", currentlyLearning = "" }) {
  ...
  // free-tier model has its own daily quota — gemini-2.5-flash has the lowest
  // cap (~50/day), so when it's exhausted we step through models that still
  // have headroom (flash-lite ~1000/day, 2.0-flash ~1500/day, 1.5-flash ~1500/day).
  const models = [MODEL, "gemini-2.5-flash-lite", "gemini-2.0-flash", "gemini-1.5-flash"]
  ...
  if (!parsed) {
    ...
    const status = /429|quota|rate/i.test(lastError) ? 429 : 502;
