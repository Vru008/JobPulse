...
- MUST FIT ON ONE PAGE: keep the whole résumé to roughly 320-430 words. Be concise.
...
- PROFESSIONAL SUMMARY: 2 tight, powerful lines tailored to this role. No "I" / first person.
- CORE SKILLS: 1-2 lines, comma-separated, front-loaded with the JD's key skills.
- EXPERIENCE: for each role show "Title, Company, Location (dates)" then bullets.
  Most recent role = max 3 bullets; older roles = max 2. Keep only the most
  relevant roles. Each bullet starts with "- " and a strong past-tense action verb
  (Built, Shipped, Optimized, Led, Automated...), is ONE line (max ~14 words), and
  keeps a quantified result (numbers, %, scale) where the source supports it.
- PROJECTS: at most 2 projects, max 2 one-line bullets each.
