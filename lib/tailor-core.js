...
function buildPrompt({ resume, company, jobDescription, gradDate, linkedin, github, confirmedSkills }) {
  ...
  const li = linkedin || CANON.linkedin;
  const gh = github || CANON.github;
...
═══ CLASSIFYING JD REQUIREMENTS (do this before scoring) ═══
For each hard requirement, put it in exactly ONE bucket:
- covered: the résumé genuinely demonstrates it (skill/experience the candidate has).
- transferable: the candidate lacks DIRECT experience but has real ADJACENT experience
  that legitimately maps to it — e.g. a "customer-facing / client communication /
  stakeholder / Customer Success" requirement maps to the candidate's real
  "collaborated with clients across multiple projects" and "cross-functional / UI-UX
  collaboration" experience. When this applies, REFRAME a real experience bullet to
  surface that adjacent experience (never invent a CS job title/metric), and list the
  requirement under "covered" tagged "(transferable)". Do NOT dump it in truly-absent.
- missing: ONLY requirements with neither direct nor honest adjacent experience.
Reserve "truly absent" for things the candidate genuinely cannot claim at all.

Return STRICT JSON:
{ "resume": string, "coverLetter": string, "atsScore": integer,
  "covered": string[], "missing": string[], "notes": string[] }
- "covered": include (transferable) items, tagged as such.
- "missing": for each, note "Tier-2 not confirmed" vs "truly absent". Contact links
  LinkedIn/GitHub are ALWAYS provided above — NEVER list them as missing.
