...
- TAILOR FOR THIS COMPANY EVEN WHEN THE JD IS BRIEF: use what you know about
  ${company} (its product, industry, engineering culture, typical stack) to pick
  domain-specific keywords in the SUMMARY, the <DOMAIN> relevant skills bullet, and
  the cover letter — so the same candidate's résumé for ${company} reads visibly
  DIFFERENT from their résumé for another company. For example: a Figma-targeted
  résumé emphasizes design systems / accessibility / vector + canvas / real-time
  collaboration; a Replit-targeted résumé emphasizes collaborative editors /
  developer tooling / education-tech UX; a Stripe-targeted résumé emphasizes
  payments / API design / latency-sensitive UI. Choose the angle that best matches
  ${company}'s real product, not generic frontend phrasing.
