...
function buildPrompt({ resume, company, jobDescription, portfolio, currentlyLearning, gradDate }) {
  ...
  const gradPart = gradDate
    ? `\n- In the EDUCATION line for the in-progress MS degree, write the status as "(Expected ${gradDate})" EXACTLY VERBATIM. NEVER write "(In Progress)", and NEVER invent a different year like "Sept 2025" — copy ${gradDate} character-for-character.`
    : `\n- gradDate was not provided. For the in-progress MS degree, DO NOT invent a date — write "(Expected — date pending)" and add a note asking the candidate to set their Expected graduation date in the Profile tab.`;
...
- Tools & Quality: <Git, GitHub, React Testing Library, Axios, validation, ...>${learningPart}${gradPart}
...
EDUCATION
<School> - <Degree, field>${gradDate ? ` (Expected ${gradDate})` : " (Expected <month YYYY — leave blank and add a note if gradDate was not provided>)"} | <details if any>
...
- If gradDate is NOT provided and EDUCATION shows an expected/"Exp." or missing
  graduation date, do NOT invent one. Add a note telling the candidate to set
  their Expected graduation date in the Profile tab.
...
async function tailor({ resume, company, jobDescription, portfolio = "", currentlyLearning = "", gradDate = "" }) {
  ...
  const body = {
    contents: [{ role: "user", parts: [{ text: buildPrompt({ resume, company, jobDescription, portfolio, currentlyLearning, gradDate }) }] }],
    generationConfig: {
      temperature: 0.6,
