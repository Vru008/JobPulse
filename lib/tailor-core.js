...
function buildPrompt({ resume, company, jobDescription, portfolio, currentlyLearning, gradDate }) {
  ...
  const gradPart = gradDate
    ? `\n- In the EDUCATION line for the in-progress MS degree, write the status as "(Expected ${gradDate})" verbatim. NEVER write "(In Progress)" — use the expected date that was provided.`
    : "";
...
EDUCATION
<School> - <Degree, field> (<status — for the in-progress MS degree write "Expected ${gradDate || "<date>"}" — never write "In Progress">) | <details if any>
