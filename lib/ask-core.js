...
async function ask({ profile, question, history }) {
  ...
  // Each Gemini free-tier model has its own daily quota — fall through to
  // wider-quota models when the primary's cap is hit.
  ...
  throw new AskError(status, `Gemini unavailable after retries: ${lastError}`);
}
