...
async function fromJSearch(query) {
  const key = process.env.JSEARCH_API_KEY;
  if (!key) return [];
  const first = (query || "Frontend Developer").split(",")[0].trim() || "Frontend Developer";
  const url = `https://jsearch.p.rapidapi.com/search?query=${encodeURIComponent(`${first} in USA`)}&page=1&num_pages=1&date_posted=week`;
 ...
 */
async function fetchJobs({ q = "", skills = "", limit = 25, seed = "", scope = "us" } = {}) {
  ...
  const grouped = {};
  scoped.forEach((j) => { (grouped[j.fitBand] || (grouped[j.fitBand] = [])).push(j); });
  const ranked = [];
  order.forEach((band) => {
