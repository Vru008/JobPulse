...
const GREENHOUSE = [
  ...
  // Added from user's top-20 watchlist (those not already covered above):
  "notion", "vercel", "hubspot", "block",
  // Atlassian uses Workday (proprietary, no free API) — cannot be watched.
  // ---- Pool-expansion batch: widely-known tech cos on Greenhouse. Invalid
  //      slugs fail silently (harmless); rotation keeps per-cycle cost flat. ----
  "1password", "algolia", "anduril", "assembled", "chainalysis", "circleci",
  "clari", "crowdstrike", "deel", "fanduel", "intercom", "klaviyo", "mixpanel",
  "opensea", "quora", "wealthsimple", "zocdoc", "zscaler", "handshake", "hopper",
  "lucid", "matterport", "medallia", "outreach", "pendo", "productboard",
  "recharge", "thoughtspot", "yotpo", "gong", "includedhealth", "kong", "mercari",
  "motive", "nextdoor", "niantic", "nuro", "quizlet", "semgrep", "shipbob",
  "sourcegraph", "teleport", "tempus", "thumbtack", "turo", "udacity", "whatnot",
  "yelp", "zuora", "gohealth", "gitlab", "netlify", "vimeo", "affirm",
];
const LEVER = [
  "matchgroup", "ro", "attentive", "kandji", "fivetran", "netlify",
  "huggingface", "metabase", "posthog", "voiceflow", "mux", "pathai",
  // pool-expansion
  "gopuff", "getir", "cloudkitchens", "swordhealth", "yugabyte", "showpad",
  "convoy", "nuvocargo", "sardine", "welcometothejungle", "kaseya", "brightwheel",
];
const ASHBY = [
  ...
  // pool-expansion: dev-tool startups (Ashby-heavy) — great for a React dev
  "supabase", "resend", "anysphere", "railway", "warp", "dagster", "prefect",
  "raycast", "thebrowsercompany", "neondatabase", "turso", "unkey", "triggerdev",
  "inngest", "liveblocks", "novu", "documenso", "calcom", "dub", "knock",
  "courier", "highlight", "hyperdx", "julius", "granola", "flightcontrol",
  "render", "tigris", "openai", "runwayml",
];

// Per-cycle rotation window. The company universe is large (300+ boards); fetching
// all of them every sweep would blow Vercel's 30s budget. Instead each sweep
// samples a deterministic contiguous window of `size` boards, offset by a hash of
// the seed. Because the cron passes an hour-granular seed, the 8am/1pm/6pm sweeps
// each see a DIFFERENT window — so across a day we cover the whole universe while
// keeping any single call's cost flat.
function rotateWindow(arr, seed, size) {
  ...
  const off = h % arr.length;
  const out = [];
  for (let i = 0; i < size; i += 1) out.push(arr[(off + i) % arr.length]);
  return out;
}

// Per-cycle board budgets — sum (~135) matches the old always-on load, so no
// timeout regression, while the total universe is now much larger.
const GH_WINDOW = 90;
const LEVER_WINDOW = 18;
const ASHBY_WINDOW = 32;
...
async function fromJSearch(query, seed) {
  ...
  // v5 endpoint = /search-v2; jobs live under data.jobs. 2 pages = wider publisher
  // mix (more chance of Indeed/Glassdoor, which Google indexes less than LinkedIn).
  const url = `https://jsearch.p.rapidapi.com/search-v2?query=${encodeURIComponent(`${first} in ${metro}`)}&num_pages=2&country=us&date_posted=week`;
...
async function fetchJobs({ q = "", skills = "", limit = 25, seed = "", scope = "us", only = "" } = {}) {
  ...
  const feedDefs = only === "jsearch"
    ...
    : [
        ...
        ...rotateWindow(GREENHOUSE, seed, GH_WINDOW).map((s) => ["greenhouse", () => fromGreenhouse(s)]),
        ...rotateWindow(LEVER, seed, LEVER_WINDOW).map((s) => ["lever", () => fromLever(s)]),
        ...rotateWindow(ASHBY, seed, ASHBY_WINDOW).map((s) => ["ashby", () => fromAshby(s)]),
      ];
  ...
  for (const job of kept) {
    ...
    if (fit < 45) continue; // loosened 55->45 so ~5 survive dedupe per cycle
                            // (user prioritizes volume; role filter in UI trims)
    job.fit = fit;
