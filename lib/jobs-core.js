 ...
 */
async function fetchJobs({ q = "", skills = "", limit = 25 } = {}) {
  ...
  const counts = { remotive: 0, arbeitnow: 0, remoteok: 0, greenhouse: 0, lever: 0, ashby: 0 };
  const excluded = {};
  let all = [];
  settled.forEach((r, i) => {
  ...
  // Rank by fit, then US/remote preference, then freshness.
  unique.sort((a, b) =>
    (b.fit - a.fit) ||
    (usScore(b) - usScore(a)) ||
