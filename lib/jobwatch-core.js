...
async function setWatch(token, body) {
  ...
  if (body && body.mode === "replace") {
    ...
    // Dedupe incoming matches against current + APPLIED only.
    // Archive is intentionally NOT in the dedupe set: with such a small free-
    // API pool, blocking archived roles forever guarantees 0 fresh after a
    // few sweeps. The cron's filter already drops only current+applied, and
    // this matches it so legitimately new picks aren't silently rejected
    // here. Applied (the user's real applications) still hard-blocks.
...
/* Clears the archive bucket only. current and applied stay intact, so the
 * next cron sweep can re-surface previously-skipped roles as Fresh while the
 * user's applied list is preserved.
 */
