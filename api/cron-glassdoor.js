/* Vercel serverless function — POST /api/cron-glassdoor
 *
 * Fires automatically from GitHub Actions (8 AM / 1 PM / 6 PM ET) so the
 * Glassdoor Watcher tab keeps getting fresh Glassdoor-sourced roles without
 * any laptop dependency.
 *
 * Glassdoor doesn't have a public job-listing API, so this uses the existing
 * JSearch (Google for Jobs) pipeline and keeps only the roles whose publisher
 * is Glassdoor — that subset of Glassdoor that Google indexes. Not 100% of
 * Glassdoor, but no scraping, no ToS issues, no extra paid keys.
 *
 * Auth: header x-cron-secret must equal env CRON_SECRET (same as cron-watcher).
 */
const { fetchJobs } = require("../lib/jobs-core");
const { setWatch, getWatch } = require("../lib/glassdoorwatch-core");
...
// Same defensive US-only filter the Indeed cron uses, so the Glassdoor list
...
// Glassdoor-published filter — JSearch tags each role with sourceName like
// "Glassdoor (Google Jobs)" when Google indexed it from Glassdoor. We also
// accept apply URLs pointing at glassdoor.com as a fallback.
function isGlassdoorRole(job) {
  const sn = String(job.sourceName || job.source || "").toLowerCase();
  const url = String(job.url || "").toLowerCase();
  if (sn.includes("glassdoor")) return true;
  if (url.includes("glassdoor.com")) return true;
  return false;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // call JSearch alone — that's where Glassdoor-published roles live.
    ...
    for (const job of candidates) {
      if (!isGlassdoorRole(job)) continue;
      const match = {
        company: job.company || "",
        role: job.title || "",
    ...
    res.status(200).json({
      ...
      glassdoorFiltered: candidates.filter(isGlassdoorRole).length,
      seenBefore: seen.size,
      currentCount: result.currentCount,
      archiveCount: result.archiveCount,
    });
  } catch (err) {
    res.status(err.status || 500).send(err.message || "cron-glassdoor failed.");
  }
};
